...
; Train
  ...
  let Fit-Train (ANN:Train Batch Ws Net 0 Xtr Ytr Xte Yte Learning-rate Number-of-epochs)
  ...
  ; Cost
  let M (item 0 matrix:dimensions Xtr)
  let Jtrain (ANN:ComputeError Ytr Otrain Ws 0)

  let Mtest 0
  let Jtest 0
  set Mtest (item 0 matrix:dimensions Xte)
  set Jtest (ANN:ComputeError Yte Otest Ws 0)


  ; Printing infos
;  print "Xtrain:"
;  print matrix:pretty-print-text Xtr
  print "Train:"
  (foreach (matrix:to-row-list Ytr) (matrix:to-row-list Otrain) [
    [y o] -> print (word (y = (discretize o)) "--" y "--" (discretize o)"--" o )])
;  print "Xtest:"
;  print matrix:pretty-print-text Xte
  print "Test:"
...
to load [f]
  ...
  let classes remove-duplicates map last data

  ; Normalize the data
  set data map [x -> (list (map [z -> z / 6 - 0.3] (bl x)) (cat-to-bin (last x) classes))] data

  ; Train Dataset
  set data-train sublist data 0 sdtrain
  set Xtr matrix:from-row-list (map first data-train)
  set Ytr matrix:from-row-list (map last data-train)

  ; Test Dataset
  set data-test sublist data sdtrain (length data)
  set Xte matrix:from-row-list (map first data-test)
