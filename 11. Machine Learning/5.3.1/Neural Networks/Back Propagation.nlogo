...
to setup-neurons
  ...
  foreach (n-values Neurons-Hidden-Layer [?]) [
    create-hidden-neurons 1 [
      setxy 2 (- int (neurons-Hidden-Layer / 2) + 1 + ?)
      set activation random-float 0.1 ]]

  ; Create Output neurons
  foreach (reverse n-values 10 [?]) [
...
to store-input
  ...
  let pattern map [[color] of ?] (sort input-neurons)
  ; Get the binary input from pattern
  set inputs map [ifelse-value (? = black) [1] [0]] pattern
