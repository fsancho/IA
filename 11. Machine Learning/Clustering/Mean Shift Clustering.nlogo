...
; We will make use of three different breed turtles to our implementation
; Data will contain the data to be clustered
breed [data datum]

; Climbers will depart from data point and will use a Gradient Ascend Algorithm to
...
climbers-own [
  point     ; The original data point it start from
...
to setup
  ...
  ; Create one climber per data point
  ask data [
    hatch-climbers 1 [
      set point myself
...
; Create the data points in a random way
to setup-data [num]
  create-data num [
...
; Create the data points grouped in a random set of bags. The number of
...
; Report de Gaussian distribution from the data points
