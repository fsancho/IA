...
; Función para crear K clusters en K puntos distintos seleccionados al azar
to crea-clusters [K]
  ask clusters [die]
  while [count clusters < K]
  [
    ask one-of points [
      if not any? clusters with [pos = [pos] of myself]
      [
        hatch 1 [
          set breed clusters
        ]
...
; Función principal:
;    K : Número de clusters para la clasificación
...
to K-medias-step
  ...
  ; Actualizamos la posición del cluster al centro geométrico de sus puntos
  ask clusters [
...
to setup
  ...
  ask patches [
    sprout-points 1 [
      set pos ifelse-value (pcolor = 0) [[0 0 0]][pcolor]
      set color pos
      ht
    ]
  ]
  show (word (length remove-duplicates [pos] of points) " colors")
end

to reduce-colores
  K-medias num-colors 5
  ask points [set pcolor [pos] of cl]
