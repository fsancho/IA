...
to-report AI:children-states
  ...
  report (map [ ?1 -> (list (swap i ?1 content) (list (word "T-" ?1) 1 "regla")) ] indexes)
end
...
;  report length filter [ ? = False] (map [?1 = ?2] c #Goal)
  report sum (map [ ?1 -> manhattan-distance (item (position ?1  c   ) pos)
                                      (item (position ?1 #Goal) pos) ]
                  (n-values 9 [ ?1 -> ?1 ]))
end
...
to test
  ...
  let path (A* In (n-values 9 [ ?1 -> ?1 ]) True True)
