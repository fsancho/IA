...
to startup
  clear-all
  let estado [1 2 3 4 5 6 7 8]
  (foreach (sort n-of 8 patches) (shuffle estado)
  [
    ask ?1
    [
      sprout-fichas 1
      [ 
        set shape word "Numero-" ?2
        set color item ?2 base-colors
        ;set color blue
...
to go
  ...
  if mouse-down?
  [
    let ficha-pulsada one-of fichas with [(distancexy mouse-xcor mouse-ycor < .5) and (member? hueco neighbors4)]
    if not (ficha-pulsada = nobody)
    [
      desplaza ficha-pulsada ([pxcor] of hueco - [xcor] of ficha-pulsada ) ([pycor] of hueco - [ycor] of ficha-pulsada )
    ]]
end
@#$#@#$#@
GRAPHICS-WINDOW
