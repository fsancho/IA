...
to-report final-state? [params]
  ...
  let res sentence (map [ ?1 -> take-row ?1 board ] [0 1 2]) (map [ ?1 -> take-column ?1 board ] [0 1 2])
...
to-report take-column [i board]
  report map [ ?1 -> item i ?1 ] board
end

to-report take-diagonal1 [board]
  report map [ ?1 -> item ?1 (item ?1 board) ] [0 1 2]
end

to-report take-diagonal2 [board]
  report map [ ?1 -> item (2 - ?1) (item ?1 board) ] [0 1 2]
end
...
to state-explorer
  if mouse-down? [
    ...
    ask selected-state [
      foreach [0 1 2] [ ?1 ->
        output-print item ?1 (first content)
      ]
