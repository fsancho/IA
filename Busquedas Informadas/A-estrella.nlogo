...
to setup
  ...
  [
    ht 
    set color (n-values 3 [100 + random 155])
    set pen-size 2
  ]
end
...
to-report Coste-total-esperado
  report Coste-camino + Heuristica 
end

; Función de patch que devuelve la heurística desde el punto actual hasta el 
;  Fin
to-report Heuristica
  report distance Fin
end
...
to A*
  ...
  [
    ...
    let opciones p-validos with [activo?]
    ; Si los hay
    ifelse any? opciones
    [
      ; Tomamos uno de los activos que tenga coste mínimo
      ask min-one-of opciones [Coste-total-esperado]
      [
        ...
        ask vecinos with [member? self p-validos]
        [
  ...
  if existe?
  [
    ; Hacemos que la tortuga recorra el camino anterior en sentido inverso (desde el 
    ;  Fin hasta el Inicio), dejando un rastro en el suelo
    let actual Fin
    ask turtles [pu move-to actual pd set color (n-values 3 [100 + random 155])]
    While [actual != Inicio]
    [
      set actual [padre] of actual
      ask turtles [move-to actual]
    ]
  ]
end
...
; Devuelve el Coste del camino calculado
to-report Coste
  report precision ([Coste-total-esperado] of Fin) 3
end
