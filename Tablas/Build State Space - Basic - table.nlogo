...
__includes [ "BSS.nls" "LayoutSpace.nls" "FP.nls"]
...
to-report actions ; [ ["*3" regla*3] ["+7" regla+7] ["-2" regla-2] ]
  let temp table:make
  table:put temp "*3" ([ x -> x * 3 ])
  table:put temp "+7" ([ x -> x + 7 ])
  table:put temp "-2" ([ x -> x - 2 ])
  report temp
end
...
to-report AI:children-states
  ...
  (table:map [ [name action] -> (list (run-result action content) name) ]
                     actions)
end
