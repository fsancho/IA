...
to-report todosEstados [c]
  ...
  let nm first first  c
  let nc last first  c
  let b last c

  ifelse b = 0
  [
    report map [(list (list (list (nm - first ?)
                                  (nc - last ?))
                             (1 - b))
                       (list ?))] movs
  ]
...
to-report valid? [c]
  let estado first c
  let nmi first first estado
  let nci last first estado
  let b last estado
  let nmd 3 - nmi
  let ncd 3 - nci
  report (((nmi >= nci) and (nmd >= ncd)) or nmi = 0 or nmd = 0) and (nci >= 0) and (ncd >= 0)
end
