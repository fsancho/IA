...
to setup
  clear-all
  set I-set n-values 20 [(random 100) - 50]
  show I-set
end

to launch
  Let Initial_State n-values 20 [random 2]
  let final AI:SimAnn Initial_State
          tries-by-cycle
          (10 ^ -6)
          cooling-rate
          accept-equal-changes?
  show (map * final I-set)
  show abs  sum (map * final I-set)
end
...
to-report AI:get-new-state [#state]
  let i random 20
  let el item i #state
  report replace-item i #state (1 - el)
end

; The energy of a state is the sum of energies of its members
to-report AI:EnergyState [#state]
  if sum #state = 0 [report 1000] ; We penalize empty solutions
  report abs sum (map * #state I-set)
end
