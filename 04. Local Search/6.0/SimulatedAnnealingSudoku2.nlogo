...
;    ]
  ...
  ask n-of prefixed patches [
    set fixed? true
    set plabel-color blue
  ]
  set changeable patches with [not fixed?]
  repeat 100 [
  ...
  ; every cell has energy 1 when it is correct, so the minimum energy is 81
...
to go
  ...
  repeat 1
  [
    let p1 rnd:weighted-one-of changeable [constraints]
    let p2 rnd:weighted-one-of changeable [constraints]
    try-swap p1 p2
    compute-constraints
    update-view
  ]
...
to try-swap [p1 p2]
  if ([value] of p1 = [value] of p2) ; si es el mismo color, no hay opción de cambio
    [ stop ]
