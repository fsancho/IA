...
to setup
  ...
  ask patches [
    set pcolor ifelse-value ((pxcor + pycor) mod 2 = 0) [white] [black]
  ]
end
...
; Here we need to give some representation to the states:
; The states are lists [a0 ... aN], and in position i, ai says that there
; is a queen in (i,ai)
...
; 1/0 to say if queens in columns i and j and threaten each other
...
to-report AI:EnergyState [#state]
  let res 0
  let lis (n-values (length #state) [?])
  foreach lis [
    let i ?
    foreach (filter [? > i] lis) [
      let j ?
