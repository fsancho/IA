...
[
  ...
  R       ; Valor de recompensa para el algoritmo Q-learning
  Q       ; Valor de aprendizaje para el algoritmo Q-learning
  desv    ; Valor de variación en el aprendizaje para el algoritmo Q-learning
]

globals [
  M ; Número de torres, se calcula a partir de la longitud del estado inicial
    ; El número de discos viene determinado por los valores usados en el estado
    ; de entrada
]
...
; Discos 1 < 2 < 3 < ... < N
; estado = [ [Palo1] [Palo2] [Palo3] ... [PaloM] ]
; Palo_i= [i_1 < i_2 < i_3], [i_1 < i_2], [i_1], [ ]
...
to-report transicion-valida? [t s]
  ...
  let cabeza first (item j s)
  if disco < cabeza [report true]
  report false
end
...
;---------------- Algoritmo contrucción del grafo de transiciones ----------------
...
; no disponer de más estados sin explorar.

to grafo-completo [estado-inicial]
  ca
  ask patches [ set pcolor white ]
  set M length read-from-string Estado_Inicial
  ...
  [
    ...
    set color green
    set size 2
    set contenido read-from-string estado-inicial
      ...
      [
        ...
        [
          ...
          [
            ...
            create-transicion-from myself [set regla ? set label first ? set label-color black]
            set color white
        ...
        [
          ...
          create-transicion-to pasado [set regla ? set label first ? set label-color black]
        ]        
...
to layout
 ;layout-radial estados transiciones estado 0
  if tam-estados != [size] of estado 0 [ask estados [set size tam-estados]]
  layout-spring estados transiciones elasticidad longitud repulsion
end

to ajusta
  ask estados [setxy (xcor * .9 + random-float 1) (ycor * .9 + random-float 1)] 
end

;;
;; Procedimientos asociados al algoritmo de Q-learning
;;

; Procedimiento de entrenamiento
to Q-learning 
  ; Reseteamos los ticks porque nos sevirán para contar el número de pasos
  reset-ticks
  ; Reiniciamos los valores de transiciones y estados
  ask transiciones [
    set Q 0
    set R 0
    set label ""
    set thickness 0
    set color grey
    set desv (Desv-maxima + 1)
  ]  
  ask estados [
    set label ""
  ]
  ; Las transiciones que llegan a los estados finales se ponen con recompensa máxima
  let ef one-of estados with [contenido = read-from-string Estado_Final]
  ask ef [
    ask my-in-transiciones [set R 100]
    set color red
  ]
  ; Repetimos el proceso de aprendizaje de las transiciones hasta que la variación
  ; máxima está por debajo de la cota
  while [max [desv] of transiciones > Desv-maxima]
  [
    ask transiciones [
      let Q2 alpha * Q + (1 - alpha) * (R + gamma * max [Q] of ([my-out-transiciones] of end2))
      set desv abs (Q2 - Q)
      set Q Q2
      set label Q
    ]
...
; Para usar la información de Q en el cálculo de la solución del problema
; basta ir moviéndonos desde el estado inicial haciendo uso de las 
; transiciones con más peso (Q) en cada estado. 
to Q-ejecuta
  ;ask transiciones [set thickness 0]
  ask estados [set label ""]
  let es one-of estados with [contenido = read-from-string Estado_Inicial]
  ask es [ set color green ]
  let ef one-of estados with [contenido = read-from-string Estado_Final]
  while [es != ef]
  [
    ask es [
      set label (word contenido "       ")
      ;write contenido
      let accion max-one-of my-out-transiciones [Q] 
      ask accion [
      ;  print (word " - " first regla " -> ")
        set color red
        set label first regla
      ]
      set es [end2] of accion
    ]
  ]
  ask ef [
    set label (word contenido "       ")
