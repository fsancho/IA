...
to-report una-f
  let new (list ((random-float 10) - 5) random 10)
  report new
end
...
to AI:ExternalUpdate
  ask AI:individuals [
    if random 2 = 0 [set content reverse content]
  ]
...
to-report apply [f x]
  report sum map [(first ?) * x ^ (last ?)] f
end

to-report  intervalo [a1 b1 ptos]
  report (n-values (1 + ptos) [a1 + ? * (b1 - a1) / ptos])
end
@#$#@#$#@
GRAPHICS-WINDOW
145
