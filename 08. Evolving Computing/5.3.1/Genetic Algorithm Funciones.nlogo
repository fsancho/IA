...
to AI:Initial-Population [#population]
  create-AI:individuals #population [
    set content map [una-f] (n-values 10 [?])
...
to AI:Compute-fitness
  set fitness 100 - sum map dif (n-values 100 [? / 100])
end
...
to AI:mutate [#mutation-ratio]
  set content map [ifelse-value (random-float 100.0 < #mutation-ratio) [una-f] [?]] content
end

to-report una-f
  let op- (list "x-y" (task [?1 - ?2]))
  let opi- (list "y-x" (task [?2 - ?1]))
  let op+ (list "x+y" (task [?1 + ?2]))
  let op* (list "x*y" (task [?1 * ?2]))
  let op/ (list "x/y" (task [ifelse-value (?2 != 0) [?1 / ?2][1]]))
  let opi/ (list "y/x" (task [ifelse-value (?1 != 0) [?2 / ?1][1]]))
  let p2 (list "y" (task [?2]))
  let c0 (list "0" (task [0]))
  let c1 (list "1" (task [1]))
  let c2 (list "2" (task [2]))
  let c3 (list "3" (task [3]))
  let c4 (list "4" (task [4]))
  let c5 (list "5" (task [5]))
  let c6 (list "6" (task [6]))
  let c7 (list "7" (task [7]))
  let c8 (list "8" (task [8]))
  let c9 (list "9" (task [9]))
...
to AI:ExternalUpdate
  ...
  foreach (n-values 100 [? / 100]) [
    let res apply f ?
    if res != false [plotxy ? res]
  ]
  plot-pen-up
  plotxy 0 0
  plot-pen-down
