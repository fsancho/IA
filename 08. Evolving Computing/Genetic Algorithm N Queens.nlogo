...
to AI:Initial-Population [#population]
  create-AI:individuals #population [
    set content n-values N [random N]
    AI:Compute-fitness
    hide-turtle
  ]
end
...
to AI:Compute-fitness
  let res 0
  let lis (n-values (length content) [?])
  foreach lis [
    let i ?
...
to AI:mutate [#mutation-ratio]
  set content map [ifelse-value (random-float 100.0 < #mutation-ratio) [random N] [?]] content
end
