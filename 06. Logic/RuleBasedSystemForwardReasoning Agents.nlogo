breed [ag-facts ag-fact]
breed [ag-rules ag-rule]

ag-facts-own [active?]
ag-rules-own [active?]

to-report deduce [G Rs Fs]
  ...
  let rep one-of ag-facts with [label = G]
  report [active?] of rep
end

to create-graph [Rs Fs G]
  ask patches [set pcolor white]
  let VP  remove-duplicates (reduce sentence Rs)
  foreach VP [
    f ->
    create-ag-facts 1 [
      set shape "circle"
  ...
  foreach Rs [
    ...
    create-ag-rules 1 [
      ...
      set label-color black
      set active? false
      create-links-from (ag-facts with [member? label (bl r)])
...
to test1
  ...
  let goal "Cebra"
  print (word "Answer: " deduce goal rules facts)
end
...
to lay
  layout-spring turtles links 0.6 4 3
end
@#$#@#$#@
GRAPHICS-WINDOW
210
...
Polygon -16777216 true false 150 30 45 240 90 240 150 105 210 240 255 240 150 30

conj2
false
15
Rectangle -1 true true 0 0 300 285
Polygon -16777216 true false 150 30 45 240 90 240 150 105 210 240 255 240 150 30
Rectangle -16777216 false false 0 0 300 285
