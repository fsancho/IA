...
to-report deduce [G Rs Fs #debug]
  ; Print the Knowledge Base
  if #debug [printKB Rs Fs]

  ; Compute the rules that are applicable with the current Facts
  let applicable-rules filter [r -> applicable? r Fs] Rs

  ; Print Applicable Rules (if #debug = true)
  if #debug [
    output-print (word "Applicable rules: ")
    foreach applicable-rules [r -> output-print (word "   " (pretty-rule r))]
    output-print " "
  ]
...
to-report applicable? [r Fs]
  let premises bl r
  report subset? premises Fs
end
; -------------------------------------------------------------------------

; Auxiliary report for subset relation

to-report subset? [A B]
...
to printKB [Rs Fs]
  ...
  foreach Rs [r -> output-print (word "   " (pretty-rule r))]
  output-print (word "Current Facts: " (pretty-facts Fs))
  output-print " "
end
; -------------------------------------------------------------------------
...
to test1
  ...
  let goal "Cebra"
  print (word "Answer: " deduce goal rules facts true)
end
