...
globals [
  ...
  grado-de-consistencia-R1
  grado-de-consistencia-R2
  grado-de-consistencia-R3

  consecuencia-R1
  consecuencia-R2
  consecuencia-R3

  conjunto-difuso-adecuacion
  adecuacion
]
...
to startup
  clear-all
  crea-conjuntos-difusos
  calcula-adecuacion
  do-plots
end


to crea-conjuntos-difusos
  set barato         fuzzy:piecewise-linear-set [[0 1] [200 0]]
  set caro           fuzzy:piecewise-linear-set [[0 0] [200 1]]
  set precio-medio   fuzzy:piecewise-linear-set [[0 0] [100 1] [200 0]]

  set cerca-del-trabajo       fuzzy:gaussian-set [0   30 [0 100]]
  set lejos-del-trabajo       fuzzy:gaussian-set [100 30 [0 100]]
  set alrededor-de-50km fuzzy:gaussian-set [50  10 [0 100]]

  set muy-adecuado    fuzzy:gaussian-set [10 2 [0 10]]
  set poco-adecuado     fuzzy:gaussian-set [0  2 [0 10]]
  set medianamente-adecuado fuzzy:gaussian-set [5  2 [0 10]]
end
...
to calcula-adecuacion

  ;; COMPUTATION OF DEGREES OF CONSISTENCY BETWEEN FACTS (INPUTS) AND ANTECEDENTS FOR EACH RULE

  ;; Rule 1: IF (House is barato OR cerca-del-trabajo)...
  let grado-de-consistencia-R1a fuzzy:evaluation-of barato precio
  let grado-de-consistencia-R1b fuzzy:evaluation-of cerca-del-trabajo distancia
  set grado-de-consistencia-R1 (runresult (word tipo-de-or " list grado-de-consistencia-R1a grado-de-consistencia-R1b"))

  ;; Rule 2: IF (House is caro OR lejos-del-trabajo)...
  let grado-de-consistencia-R2a fuzzy:evaluation-of caro precio
  let grado-de-consistencia-R2b fuzzy:evaluation-of lejos-del-trabajo distancia
  set grado-de-consistencia-R2 (runresult (word tipo-de-or " list grado-de-consistencia-R2a grado-de-consistencia-R2b"))

  ;; Rule 3: IF (House is precio-medio AND About-50-km-from-work)...
  let grado-de-consistencia-R3a fuzzy:evaluation-of precio-medio precio
  let grado-de-consistencia-R3b fuzzy:evaluation-of alrededor-de-50km distancia
  set grado-de-consistencia-R3 (runresult (word tipo-de-and " list grado-de-consistencia-R3a grado-de-consistencia-R3b"))


  ;; COMPUTATION OF RESHAPED CONSEQUENTS FOR EACH RULE

  ;; Rule 1: ... THEN Suitability is Good.
  set consecuencia-R1 (runresult (word "fuzzy:" tipo-implicacion " muy-adecuado grado-de-consistencia-R1"))

  ;; Rule 2: ... THEN Suitability is Low.
  set consecuencia-R2 (runresult (word "fuzzy:" tipo-implicacion " poco-adecuado grado-de-consistencia-R2"))

  ;; Rule 3: ... THEN Suitability is Regular.
  set consecuencia-R3 (runresult (word "fuzzy:" tipo-implicacion " medianamente-adecuado grado-de-consistencia-R3"))


  ;; AGGREGATION OF ALL THE RESHAPED CONSEQUENTS
  set conjunto-difuso-adecuacion (runresult (word "fuzzy:" tipo-agregacion " (list consecuencia-R1 consecuencia-R2 consecuencia-R3)"))


  ;; DEFUZZIFICATION OF THE AGGREGATED FUZZY SET
  set adecuacion (runresult (word "fuzzy:" tipo-desborrificacion "-of conjunto-difuso-adecuacion"))
...
to do-plots
  ...
  set-current-plot "barato"
  draw barato precio 200

  set-current-plot "caro"
  draw caro precio 200

  set-current-plot "precio-medio"
  draw precio-medio precio 200

  set-current-plot "cerca-del-trabajo"
  draw cerca-del-trabajo distancia 100

  set-current-plot "lejos-del-trabajo"
  draw lejos-del-trabajo distancia 100

  set-current-plot "alrededor-de-50km"
  draw alrededor-de-50km distancia 100

  set-current-plot "muy-adecuado"
    fuzzy:plot muy-adecuado
    set-current-plot-pen "green"
    fuzzy:plot consecuencia-R1

  set-current-plot "poco-adecuado"
    fuzzy:plot poco-adecuado
    set-current-plot-pen "green"
    fuzzy:plot consecuencia-R2

  set-current-plot "medianamente-adecuado"
    fuzzy:plot medianamente-adecuado
    set-current-plot-pen "green"
    fuzzy:plot consecuencia-R3

  set-current-plot "adecuacion-agregada" fuzzy:plot conjunto-difuso-adecuacion
