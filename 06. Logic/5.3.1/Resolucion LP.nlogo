...
to-report lit-resolventes [c1 c2]
  let res []
  foreach c1 [
    if member? (op ?) c2 [set res lput ? res]
  ]
  report res
end
...
to-report escribe-conjunto [c]
  report (word "{ { " 
               (reduce [word ?1 (word " }, { " ?2)] (map escribe-clausula c))
               " } }")
end
...
to procesa
  ...
  ask Clausulas-Actual [
    ...
    ask other Clausulas-Actual with [resolubles? c1 contenido] [
      foreach lit-resolventes c1 contenido [
        let nuevaClausula limpia (resolvente c1 contenido ?)
